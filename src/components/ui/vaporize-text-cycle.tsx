...
export default function VaporizeTextCycle({
  ...
  const animationDurations = useMemo(() => ({
    ...
    INITIAL_DELAY: (animation.initialDelayMs ?? 0),
...
const renderCanvas = ({
  ...
  // Clamp font size to container height to prevent clipping
  const clampedFontSize = Math.min(fontSize * globalDpr, Math.floor((wrapperSize.height * 0.7)));
  const font = `${framerProps.font?.fontWeight ?? 400} ${clampedFontSize}px ${framerProps.font?.fontFamily ?? "sans-serif"}`;
  ...
  // If text width exceeds canvas, reduce DPR rendering scale to avoid cropping while keeping visual width
  if (textBoundaries.width > canvas.width * 0.95) {
    // Scale down font proportionally and re-render
    const scale = (canvas.width * 0.95) / textBoundaries.width;
    const adjustedPx = Math.floor(clampedFontSize * scale);
    const adjustedFont = `${framerProps.font?.fontWeight ?? 400} ${adjustedPx}px ${framerProps.font?.fontFamily ?? "sans-serif"}`;
    const { particles: p2, textBoundaries: t2 } = createParticles(ctx, canvas, currentText, textX, textY, adjustedFont, color, framerProps.alignment || "center");
    particlesRef.current = p2 as unknown as Particle[];
    canvas.textBoundaries = t2;
    return;
  }

  // Store particles and text boundaries for animation
  particlesRef.current = particles as unknown as Particle[];
  ...
  particles.forEach(particle => {
    ...
    if (shouldVaporize) {
      ...
      if (particle.shouldFadeQuickly) {
        ...
        // Damping factor increases with distance
        ...
        // Slower fade out for more persistence
